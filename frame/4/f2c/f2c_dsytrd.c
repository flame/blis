/* f2c_dsytrd.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 static bla_double c_b22 = -1.;
 static bla_double c_b23 = 1.;
 /* > \brief \b DSYTRD */
 /* =========== DOCUMENTATION =========== */
 ...
 /* > \verbatim */
 /* > */
 /* > DSYTRD reduces a bla_real symmetric matrix A to bla_real symmetric */
 ...
 *info = -9;
 ...
 nb = f2c_ilaenv_(&c__1, "DSYTRD", uplo, n, &c_n1, &c_n1, &c_n1, (ftnlen)6, (ftnlen)1);
 lwkopt = *n * nb;
 ...
 /* Computing MAX */
 i__1 = nb, i__2 = f2c_ilaenv_(&c__3, "DSYTRD", uplo, n, &c_n1, &c_n1, & c_n1, (ftnlen)6, (ftnlen)1);
 ...
 i__1 = *lwork / ldwork;
 nb = bla_a_max(i__1,1);
 nbmin = f2c_ilaenv_(&c__2, "DSYTRD", uplo, n, &c_n1, &c_n1, &c_n1, (ftnlen)6, (ftnlen)1);
 if (nb < nbmin) {
 nx = *n;
 }
 }
 ...
 /* the matrix */
 i__3 = i__ + nb - 1;
 f2c_dlatrd_(uplo, &i__3, &nb, &a[a_offset], lda, &e[1], &tau[1], & work[1], &ldwork, (ftnlen)1);
 ...
 f2c_dsytd2_(uplo, &kk, &a[a_offset], lda, &d__[1], &e[1], &tau[1], &iinfo, (ftnlen)1);
 }
 else {
 ...
 /* the matrix */
 i__3 = *n - i__ + 1;
 f2c_dlatrd_(uplo, &i__3, &nb, &a[i__ + i__ * a_dim1], lda, &e[i__], & tau[i__], &work[1], &ldwork, (ftnlen)1);
 ...
 /* Use unblocked code to reduce the last or only block */
 i__1 = *n - i__ + 1;
 f2c_dsytd2_(uplo, &i__1, &a[i__ + i__ * a_dim1], lda, &d__[i__], &e[i__], &tau[i__], &iinfo, (ftnlen)1);
 }
 work[1] = (bla_double) lwkopt;
 return 0;
 /* End of DSYTRD */
