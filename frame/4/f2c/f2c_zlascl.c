/* f2c_zlascl.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 on Linux or Unix systems, link with .../path/to/libf2c.a -lm or, if you install libf2c.a in a standard place, with -lf2c -lm -- in that order, at the end of the command line, as in cc *.o -lf2c -lm Source for libf2c is in /netlib/f2c/libf2c.zip, e.g., http://www.netlib.org/f2c/libf2c.zip */
 #include "blis.h" /* > \brief \b ZLASCL multiplies a general rectangular matrix by a bla_real scalar defined as cto/cfrom. */
 /* =========== DOCUMENTATION =========== */
 ...
 /* > \verbatim */
 /* > */
 /* > ZLASCL multiplies the M by N bla_scomplex matrix A by the bla_real scalar */
 ...
 *info = -1;
 ...
 else if (*cfrom == 0. || f2c_disnan_(cfrom)) {
 *info = -4;
 }
 else if (f2c_disnan_(cto)) {
 *info = -5;
 }
 else if (*m < 0) {
 *info = -6;
 ...
 *info = -9;
 ...
 else if (bla_d_abs(cfrom1) > bla_d_abs(ctoc) && ctoc != 0.) {
 mul = smlnum;
 done = FALSE_;
 cfromc = cfrom1;
 }
 else if (bla_d_abs(cto1) > bla_d_abs(cfromc)) {
 mul = bignum;
 done = FALSE_;
 ctoc = cto1;
 }
 ...
 i__3 = i__ + j * a_dim1;
 i__4 = i__ + j * a_dim1;
 z__1.real = mul * a[i__4].real, z__1.imag = mul * a[i__4].imag;
