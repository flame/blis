/* f2c_dsyev.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 static bla_double c_b17 = 1.;
 /* > \brief <b> DSYEV computes the eigenvalues and, optionally, the left and/or right eigenvectors for SY matr ices</b> */
 /* =========== DOCUMENTATION =========== */
 ...
 *info = -8;
 ...
 anrm = f2c_dlansy_("M", uplo, n, &a[a_offset], lda, &work[1], (ftnlen)1, (ftnlen)1);
 iscale = 0;
 if (anrm > 0. && anrm < rmin) {
 iscale = 1;
 sigma = rmin / anrm;
 ...
 if (iscale == 1) {
 f2c_dlascl_(uplo, &c__0, &c__0, &c_b17, &sigma, n, n, &a[a_offset], lda, info, (ftnlen)1);
 }
 ...
 f2c_dsytrd_(uplo, n, &a[a_offset], lda, &w[1], &work[inde], &work[indtau], & work[indwrk], &llwork, &iinfo, (ftnlen)1);
 /* For eigenvalues only, call DSTERF. For eigenvectors, first call */
 /* DORGTR to generate the orthogonal matrix, then call DSTEQR. */
 if (! wantz) {
 f2c_dsterf_(n, &w[1], &work[inde], info);
 }
 else {
 f2c_dorgtr_(uplo, n, &a[a_offset], lda, &work[indtau], &work[indwrk], & llwork, &iinfo, (ftnlen)1);
 f2c_dsteqr_(jobz, n, &w[1], &work[inde], &a[a_offset], lda, &work[indtau], info, (ftnlen)1);
 }
