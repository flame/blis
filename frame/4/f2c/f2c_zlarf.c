/* f2c_zlarf.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
;
 ...
 /* > \verbatim */
 /* > */
 /* > ZLARF applies a bla_scomplex elementary reflector H to a bla_scomplex M-by-N */
...
;
 ...
 i__1 = i__;
 if (!(lastv > 0 && (v[i__1].real == 0. && v[i__1].imag == 0.))) break;
 --lastv;
 i__ -= *incv;
 }
 if (applyleft) {
 /* Scan for the last non-zero column in C(1:lastv,:). */
 lastc = f2c_ilazlc_(&lastv, n, &c__[c_offset], ldc);
 }
 else {
 /* Scan for the last non-zero row in C(:,1:lastv). */
 lastc = f2c_ilazlr_(m, &lastv, &c__[c_offset], ldc);
 }
 }
 ...
 z__1.real = -tau->real, z__1.imag = -tau->imag;
