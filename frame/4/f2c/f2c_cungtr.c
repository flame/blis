/* f2c_cungtr.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 /* > \verbatim */
 /* > */
 /* > CUNGTR generates a bla_scomplex unitary matrix Q which is defined as the */
 ...
 *info = -7;
 ...
 i__2 = *n - 1;
 i__3 = *n - 1;
 nb = f2c_ilaenv_(&c__1, "CUNGQL", " ", &i__1, &i__2, &i__3, &c_n1, (ftnlen)6, (ftnlen)1);
 }
 else {
 i__1 = *n - 1;
 i__2 = *n - 1;
 i__3 = *n - 1;
 nb = f2c_ilaenv_(&c__1, "CUNGQR", " ", &i__1, &i__2, &i__3, &c_n1, (ftnlen)6, (ftnlen)1);
 }
 /* Computing MAX */
 i__1 = 1, i__2 = *n - 1;
 ...
 i__2 = *n - 1;
 i__3 = *n - 1;
 f2c_cungql_(&i__1, &i__2, &i__3, &a[a_offset], lda, &tau[1], &work[1], lwork, &iinfo);
 }
 else {
 ...
 --j) {
 i__1 = j * a_dim1 + 1;
 a[i__1].real = 0.f, a[i__1].imag = 0.f;
 i__1 = *n;
 for (i__ = j + 1;
 i__ <= i__1;
 ++i__) {
 ...
 i__2 = *n - 1;
 i__3 = *n - 1;
 f2c_cungqr_(&i__1, &i__2, &i__3, &a[(a_dim1 << 1) + 2], lda, &tau[1], &work[1], lwork, &iinfo);
 }
 }
 work[1].real = (bla_real) lwkopt, work[1].imag = 0.f;
 return 0;
