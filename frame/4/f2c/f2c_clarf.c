/* f2c_clarf.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
;
 ...
 /* > \verbatim */
 /* > */
 /* > CLARF applies a bla_scomplex elementary reflector H to a bla_scomplex M-by-N */
 /* > matrix C, from either the left or the right. H is represented in the */
 /* > form */
 /* > */
 /* > H = I - tau * v * v**H */
 /* > */
 /* > where tau is a bla_scomplex scalar and v is a bla_scomplex vector. */
 /* > */
 ...
 bla_integer c_dim1, c_offset, i__1;
 bla_scomplex q__1;
 /* Local variables */
 bla_integer i__;
...
;
 ...
 i__1 = i__;
 if (!(lastv > 0 && (v[i__1].real == 0.f && v[i__1].imag == 0.f))) break;
 --lastv;
 i__ -= *incv;
 }
 if (applyleft) {
 /* Scan for the last non-zero column in C(1:lastv,:). */
 lastc = f2c_ilaclc_(&lastv, n, &c__[c_offset], ldc);
 }
 else {
 /* Scan for the last non-zero row in C(:,1:lastv). */
 lastc = f2c_ilaclr_(m, &lastv, &c__[c_offset], ldc);
 }
 }
 ...
 q__1.real = -tau->real, q__1.imag = -tau->imag;
