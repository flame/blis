/* f2c_zsteqr.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = -6;
 ...
 if (icompz == 2) {
 i__1 = z_dim1 + 1;
 z__[i__1].real = 1., z__[i__1].imag = 0.;
 }
 return 0;
 }
 ...
 /* matrix. */
 if (icompz == 2) {
 f2c_zlaset_("Full", n, n, &c_b1, &c_b2, &z__[z_offset], ldz, (ftnlen)4);
 }
 nmaxit = *n * 30;
 jtot = 0;
 ...
 i__1 = lend - l + 1;
 anorm = f2c_dlanst_("I", &i__1, &d__[l], &e[l], (ftnlen)1);
 iscale = 0;
 if (anorm == 0.) {
 goto L10;
 }
 ...
 work[l] = c__;
 work[*n - 1 + l] = s;
 f2c_zlasr_("R", "V", "B", n, &c__2, &work[l], &work[*n - 1 + l], & z__[l * z_dim1 + 1], ldz, (ftnlen)1, (ftnlen)1, (ftnlen)1);
 }
 else {
 ...
 if (icompz > 0) {
 mm = m - l + 1;
 f2c_zlasr_("R", "V", "B", n, &mm, &work[l], &work[*n - 1 + l], &z__[l * z_dim1 + 1], ldz, (ftnlen)1, (ftnlen)1, (ftnlen)1);
 }
 d__[l] -= p;
 e[l] = g;
 goto L40;
 ...
 work[m] = c__;
 work[*n - 1 + m] = s;
 f2c_zlasr_("R", "V", "F", n, &c__2, &work[m], &work[*n - 1 + m], & z__[(l - 1) * z_dim1 + 1], ldz, (ftnlen)1, (ftnlen)1, (ftnlen)1);
 }
 else {
 ...
 if (icompz > 0) {
 mm = l - m + 1;
 f2c_zlasr_("R", "V", "F", n, &mm, &work[m], &work[*n - 1 + m], &z__[m * z_dim1 + 1], ldz, (ftnlen)1, (ftnlen)1, (ftnlen)1);
 }
 d__[l] -= p;
 e[lm1] = g;
 goto L90;
