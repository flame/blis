/* f2c_clanhe.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 on Linux or Unix systems, link with .../path/to/libf2c.a -lm or, if you install libf2c.a in a standard place, with -lf2c -lm -- in that order, at the end of the command line, as in cc *.o -lf2c -lm Source for libf2c is in /netlib/f2c/libf2c.zip, e.g., http://www.netlib.org/f2c/libf2c.zip */
 #include "blis.h" /* Table of constant values */
 static bla_integer c__1 = 1;
 /* > \brief \b CLANHE returns the value of the 1-norm, or the Frobenius norm, or the infinity norm, or the ele ment of largest absolute value of a bla_scomplex Hermitian matrix. */
 /* =========== DOCUMENTATION =========== */
 ...
 /* > \verbatim */
 /* > */
 /* > CLANHE = ( bla_a_max(bla_r_abs(A(i,j))), NORM = 'M' or 'm' */
 /* > ( */
 ...
 /* > \author NAG Ltd. */
 /* > \ingroup bla_scomplexHEauxiliary */
 /* ===================================================================== */
 ...
 i__2 = j + j * a_dim1;
 sum = (r__1 = a[i__2].real, bla_r_abs(r__1));
 if (value < sum || f2c_sisnan_(&sum)) {
 value = sum;
 }
 /* L20: */
 ...
 i__ <= i__2;
 ++i__) {
 absa = bla_c_abs(&a[i__ + j * a_dim1]);
 sum += absa;
 work[i__] += absa;
 /* L50: */
 }
 i__2 = j + j * a_dim1;
 work[j] = sum + (r__1 = a[i__2].real, bla_r_abs(r__1));
 /* L60: */
 }
 i__1 = *n;
 for (i__ = 1;
 ...
 ++j) {
 i__2 = j + j * a_dim1;
 sum = work[j] + (r__1 = a[i__2].real, bla_r_abs(r__1));
 i__2 = *n;
 for (i__ = j + 1;
 i__ <= i__2;
 ++i__) {
 ...
 colssq[1] = 1.f;
 i__2 = j - 1;
 f2c_classq_(&i__2, &a[j * a_dim1 + 1], &c__1, colssq, &colssq[1]);
 ...
 colssq[1] = 1.f;
 i__2 = *n - j;
 f2c_classq_(&i__2, &a[j + 1 + j * a_dim1], &c__1, colssq, &colssq[ 1]);
 f2c_scombssq_(ssq, colssq);
 /* L120: */
 }
 }
 ...
 if (a[i__2].real != 0.f) {
 i__2 = i__ + i__ * a_dim1;
 absa = (r__1 = a[i__2].real, bla_r_abs(r__1));
 if (ssq[0] < absa) {
 /* Computing 2nd power */
 r__1 = ssq[0] / absa;
