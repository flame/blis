/* f2c_dorg2r.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 /* > \verbatim */
 /* > */
 /* > DORG2R generates an m by n bla_real matrix Q with orthonormal columns, */
 ...
 *info = -5;
 ...
 i__1 = *m - i__ + 1;
 i__2 = *n - i__;
 f2c_dlarf_("Left", &i__1, &i__2, &a[i__ + i__ * a_dim1], &c__1, &tau[ i__], &a[i__ + (i__ + 1) * a_dim1], lda, &work[1], (ftnlen)4);
 }
 if (i__ < *m) {
 i__1 = *m - i__;
 d__1 = -tau[i__];
