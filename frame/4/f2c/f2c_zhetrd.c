/* f2c_zhetrd.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 /* > \verbatim */
 /* > */
 /* > ZHETRD reduces a bla_scomplex Hermitian matrix A to bla_real symmetric */
 ...
 */
 ...
 /* > \author NAG Ltd. */
 /* > \ingroup bla_scomplex16HEcomputational */
 /* > \par Further Details: */
 /* ===================== */
 /* > */
 /* > \verbatim */
 ...
 *info = -9;
 ...
 /* Computing MAX */
 i__1 = nb, i__2 = f2c_ilaenv_(&c__3, "ZHETRD", uplo, n, &c_n1, &c_n1, & c_n1, (ftnlen)6, (ftnlen)1);
 ...
 i__1 = *lwork / ldwork;
 nb = bla_a_max(i__1,1);
 nbmin = f2c_ilaenv_(&c__2, "ZHETRD", uplo, n, &c_n1, &c_n1, &c_n1, (ftnlen)6, (ftnlen)1);
 if (nb < nbmin) {
 nx = *n;
 }
 }
 ...
 /* the matrix */
 i__3 = i__ + nb - 1;
 f2c_zlatrd_(uplo, &i__3, &nb, &a[a_offset], lda, &e[1], &tau[1], & work[1], &ldwork, (ftnlen)1);
 ...
 i__4 = j - 1 + j * a_dim1;
 i__5 = j - 1;
 a[i__4].real = e[i__5], a[i__4].imag = 0.;
 i__4 = j;
 i__5 = j + j * a_dim1;
 ...
 f2c_zhetd2_(uplo, &kk, &a[a_offset], lda, &d__[1], &e[1], &tau[1], &iinfo, (ftnlen)1);
 }
 else {
 ...
 /* the matrix */
 i__3 = *n - i__ + 1;
 f2c_zlatrd_(uplo, &i__3, &nb, &a[i__ + i__ * a_dim1], lda, &e[i__], & tau[i__], &work[1], &ldwork, (ftnlen)1);
 ...
 /* Use unblocked code to reduce the last or only block */
 i__1 = *n - i__ + 1;
 f2c_zhetd2_(uplo, &i__1, &a[i__ + i__ * a_dim1], lda, &d__[i__], &e[i__], &tau[i__], &iinfo, (ftnlen)1);
 }
 work[1].real = (bla_double) lwkopt, work[1].imag = 0.;
 return 0;
 /* End of ZHETRD */
