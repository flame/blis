...
double libblis_test_her2k_check(
...
double cblas_her2k(
  ...
  switch( dt )  {
    ...
    case BLIS_SCOMPLEX :
    {
      ...
      cblas_cher2k( cblas_order, cblas_uploc, cblas_trans, mm, kk, alphap,
                                      ap, lda, bp, ldb, *betap, cp, ldc );
      break;
    }
    case BLIS_DCOMPLEX :
    {
      ...
      cblas_zher2k( cblas_order, cblas_uploc, cblas_trans, mm, kk, alphap,
                                      ap, lda, bp, ldb, *betap, cp, ldc );
      break;
    }
    default :
...
double blas_her2k(
  ...
  switch( dt )  {
    ...
    case BLIS_SCOMPLEX :
    {
      ...
      float*  betap     = (float*) bli_obj_buffer( beta );
      scomplex*  cp     = (scomplex*) bli_obj_buffer( c );
      cher2k_( &f77_uploc, &f77_trans, &mm, &kk, alphap, ap, (f77_int*)&lda,
                            bp, (f77_int*)&ldb, betap, cp, (f77_int*)&ldc );
      break;
    }
    case BLIS_DCOMPLEX :
    {
      ...
      double*  betap    = (double*) bli_obj_buffer( beta );
      dcomplex*  cp     = (dcomplex*) bli_obj_buffer( c );
      zher2k_( &f77_uploc, &f77_trans, &mm, &kk, alphap, ap, (f77_int*)&lda,
                            bp, (f77_int*)&ldb, betap, cp, (f77_int*)&ldc );
      break;
    }
    default :
  ...
  if(params->api == API_BLIS) {
   ...
   if( bli_obj_row_stride( c ) == 1 ) {
    ...
    if(params->api == API_CBLAS) {
      cblas_her2k( uploc, trans, m, k, alpha, a, lda, b, ldb, beta, c, ldc, dt );
    } else {
      if( bli_obj_row_stride( c ) == 1 ) {
        blas_her2k( uploc, trans, m, k, alpha, a, lda, b, ldb, beta, c, ldc, dt );
      }
      else {
        if( uploc == BLIS_UPPER)
          uploc = BLIS_LOWER;
...
double libblis_ref_her2k(
  ...
  if ((params->nanf) && (*betap == 0)) {
    resid = libblis_check_nan_her2k(c, dt );
  }
  else if((params->bitextf == 0) && (params->oruflw == BLIS_DEFAULT)) {
	   resid = libblis_test_her2k_check( params, alpha, a, b, beta, c, c_save );
  }
  else {
    if(params->oruflw == BLIS_DEFAULT) {
	     resid = libblis_test_iher2k_check( params, alpha, a, b, beta, c, c_save );
    }
    else {
...
double libblis_test_bitrp_her2k(
  test_params_t* params,
  iface_t        iface,
  obj_t*         alpha,
  obj_t*         a,
  ...
  for(i = 0; i < n_repeats; i++) {
    bli_copym( c_orig, r );
		  libblis_test_her2k_impl( iface, alpha, a, b, beta, r );
...
double libblis_test_op_her2k (
  ...
  bli_param_map_char_to_herk_trans( pc_str[1], &trans );
  ...
  libblis_api_her2k(params, iface, &alpha, &aa, &bb, &beta, &c, datatype );
  ...
  if(params->bitrp) {
    ...
    resid = libblis_test_bitrp_her2k( params, iface, &alpha, &a, &b,
                                            &beta, &c, &c_save, &r, datatype);
    bli_obj_free( &r );
  }
  else {
    resid = libblis_ref_her2k(params, &alpha, &a, &b, &beta,
                                              &c, &c_save, datatype );
  }
#endif
