...
// Local prototypes.
void libblis_test_subv_deps(thread_data_t* tdata,
                            test_params_t* params, test_op_t* op );

void libblis_test_subv_impl(iface_t iface, obj_t* x, obj_t* y );

double libblis_test_subv_check (
...
double libblis_ref_subv(
  ...
  if((params->bitextf == 0) && (params->oruflw == BLIS_DEFAULT)) {
    resid = libblis_test_subv_check(params, alpha, beta, x, y );
  }
  else {
   if(params->oruflw == BLIS_DEFAULT) {
      resid = libblis_test_isubv_check(params, alpha, beta, x, y, y_save);
    }
    else {
...
double libblis_test_bitrp_subv(
  test_params_t* params,
  iface_t        iface,
  obj_t*         x,
  obj_t*         y,
  ...
  for(i = 0; i < n_repeats; i++) {
    bli_copyv( y_orig, r );
    libblis_test_subv_impl( iface, x, r );
...
double libblis_test_op_subv (
  ...
  if(params->bitrp) {
    ...
    resid = libblis_test_bitrp_subv( params, iface, &x, &y, &y_save, &r, datatype);

    bli_obj_free( &r );
  }
  else {
    resid = libblis_ref_subv( params, &alpha, &beta, &x, &y, &y_save);
  }
#endif
