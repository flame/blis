...
static void aocl_gemm_driver_u8s8s32os8(
  uint32_t       n_repeats,
  char           stor_order,
  char           transa,
  char           transb,
  ...
  for ( i = 0; i < n_repeats; i++ ) {
    memset( ( void* ) c, 0, sizeof( int8_t ) * (m * n) );

    aocl_gemm_u8s8s32os8(storage, transa, transb, m, n, k,
                          alpha, a, lda, reordera, b,
                          ldb, reorderb, beta, c, ldc, post_op );
/*
    aocl_gemm_u8s8s32os8(storage, (const char)transa, (const char)transb, (const dim_t)m, (const dim_t)n, (const dim_t)k,
...
static void mat_mul_driver_u8s8s32os8(
  ...
  if ( ( params->op_t == 'p' ) || ( params->op_t == 'P' ) )
  {
    ...
    aocl_gemm_driver_u8s8s32os8(params->n_repeats, stor_order, transa, transb, m, n, k,
...
static double mat_mul_accuracy_check_driver_u8s8s32os8
    (
    ...
    )
{
  ...
  resid = mat_mul_accuracy_check_driver<uint8_t,int8_t,int8_t,int32_t,float>( m, n, k,
...
double libblis_test_op_gemm_u8s8s32os8 (
  ...
  int8_t* c = ( int8_t* ) malloc( sizeof( int8_t ) * m * n );
  memset( ( void* ) c, 0, sizeof( int8_t ) * m * n );

  int8_t* c_ref = ( int8_t* ) malloc( sizeof( int8_t ) * m * n );
  memset( ( void* ) c_ref, 0, sizeof( int8_t ) * m * n );
  ...
  mat_mul_driver_u8s8s32os8(params, stor_order, transa, transb, m, n, k, alpha,
                            a, lda, b, ldb, beta, c, ldc, post_op );

#ifndef __GTEST_VALGRIND_TEST__
  resid = mat_mul_accuracy_check_driver_u8s8s32os8( stor_order, m, n, k, alpha,
