...
double libblis_test_scal2m_check(
  test_params_t* params,
  obj_t*         alpha,
  obj_t*         x,
  obj_t*         y,
...
double libblis_ref_scal2m(
  ...
  if((params->bitextf == 0) && (params->oruflw == BLIS_DEFAULT)) {
    resid = libblis_test_scal2m_check( params, alpha, x, y, y_orig);
  }
  else {
    if(params->oruflw == BLIS_DEFAULT) {
      resid = libblis_test_iscal2m_check( params, alpha, x, y, y_orig);
    }
    else {
...
double libblis_test_bitrp_scal2m(
  test_params_t* params,
  iface_t        iface,
  obj_t*         alpha,
  obj_t*         x,
  ...
  for(i = 0; i < n_repeats; i++) {
    bli_copym( y_orig, r );
    libblis_test_scal2m_impl( iface, alpha, x, r );
...
double libblis_test_op_scal2m(
  ...
  if(params->bitrp) {
    ...
    resid = libblis_test_bitrp_scal2m( params, iface, &alpha, &x,
                                           &y, &y_save, &r, datatype);

    bli_obj_free( &r );
  }
  else {
    resid = libblis_ref_scal2m( params, &alpha, &x, &y, &y_save);
  }
#endif
