...
double libblis_test_trsm_check (
  test_params_t* params,
  side_t         side,
  obj_t*         alpha,
  obj_t*         a,
...
double cblas_trsm(
  ...
  switch( dt )  {
    case BLIS_FLOAT :
    {
      ...
      cblas_strsm( cblas_order, cblas_side, cblas_uplo, cblas_transa,
    ...
    case BLIS_DOUBLE :
    {
      ...
      cblas_dtrsm( cblas_order, cblas_side, cblas_uplo, cblas_transa,
                       cblas_diag, mm, nn, *alphap, ap, lda, bp, ldb );
      break;
    }
    case BLIS_SCOMPLEX :
    {
      ...
      cblas_ctrsm( cblas_order, cblas_side, cblas_uplo, cblas_transa,
                       cblas_diag, mm, nn, alphap, ap, lda, bp, ldb );
      break;
    }
    case BLIS_DCOMPLEX :
    {
      ...
      cblas_ztrsm( cblas_order, cblas_side, cblas_uplo, cblas_transa,
                       cblas_diag, mm, nn, alphap, ap, lda, bp, ldb );
      break;
    }
    default :
...
double blas_trsm(
  ...
  switch( dt )  {
    case BLIS_FLOAT :
    {
      ...
      float*   ap     = (float*) bli_obj_buffer( a );
      float*   bp     = (float*) bli_obj_buffer( b );
      strsm_( &f77_side, &f77_uploa, &f77_transa, &f77_diaga,
    ...
    case BLIS_DOUBLE :
    {
      ...
      double*  ap     = (double*) bli_obj_buffer( a );
      double*  bp     = (double*) bli_obj_buffer( b );
      dtrsm_( &f77_side, &f77_uploa, &f77_transa, &f77_diaga,
                          &mm, &nn, alphap, ap, &lda, bp, &ldb );
      break;
    }
    case BLIS_SCOMPLEX :
    {
      ...
      scomplex*  ap     = (scomplex*) bli_obj_buffer( a );
      scomplex*  bp     = (scomplex*) bli_obj_buffer( b );
      ctrsm_( &f77_side, &f77_uploa, &f77_transa, &f77_diaga,
                          &mm, &nn, alphap, ap, &lda, bp, &ldb );
      break;
    }
    case BLIS_DCOMPLEX :
    {
      ...
      dcomplex*  ap     = (dcomplex*) bli_obj_buffer( a );
      dcomplex*  bp     = (dcomplex*) bli_obj_buffer( b );
      ztrsm_( &f77_side, &f77_uploa, &f77_transa, &f77_diaga,
                          &mm, &nn, alphap, ap, &lda, bp, &ldb );
      break;
    }
    default :
...
void libblis_api_trsm(
  ...
  if(params->api == API_BLIS) {
    ...
    if(params->api == API_CBLAS) {
      cblas_trsm(mm, nn, lda, ldb, a, b, alpha,
                                  uploa, side, diaga, transa, dt);
    } else { /**/
      if( bli_obj_row_stride( a ) == 1 ) {
        blas_trsm(mm, nn, lda, ldb, a, b, alpha,
                                uploa, side, diaga, transa, dt);
      }
      else {
        if( side == BLIS_LEFT)
        ...
        blas_trsm(nn, mm, lda, ldb, a, b, alpha,
                                uploa, side, diaga, transa, dt);
      }
    }
  }
...
double libblis_ref_trsm(
  ...
  if (params->nanf) {
    resid = libblis_check_nan_trsm(b, dt );
  }
  else if((params->bitextf == 0) && (params->oruflw == BLIS_DEFAULT)) {
    resid = libblis_test_trsm_check(params, side, alpha, a, b, b_orig);
  }
  else {
    if(params->oruflw == BLIS_DEFAULT) {
      resid = libblis_test_itrsm_check(params, side, alpha, a, b, b_orig, dt);
    }
    else {
...
double libblis_test_bitrp_trsm(
  test_params_t* params,
  iface_t        iface,
  side_t         side,
  obj_t*         alpha,
  ...
  for(i = 0; i < n_repeats; i++) {
    bli_copym( b_orig, r );
    libblis_test_trsm_impl( iface, side, alpha, a, r );
...
double libblis_test_op_trsm (
  ...
  libblis_api_trsm(params, iface, side, &alpha, &aa, &b, datatype);
  ...
  if(params->bitrp) {
    ...
    libblis_test_mobj_create( params, datatype, BLIS_NO_TRANSPOSE,
                                                     sc_str[0], m, n, &r );
    resid = libblis_test_bitrp_trsm(params, iface, side, &alpha, &a,
                                                &b, &b_save, &r, datatype);
    bli_obj_free( &r );
  }
  else {
    resid = libblis_ref_trsm(params, side, &alpha, &a, &b, &b_save, datatype);
  }
#endif
