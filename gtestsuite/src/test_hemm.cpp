...
double cblas_hemm(
  ...
  switch( dt )  {
    ...
    case BLIS_SCOMPLEX :
    {
      ...
      cblas_chemm( cblas_order, cblas_side, cblas_uplo, mm, nn, alphap,
                                      ap, lda, bp, ldb, betap, cp, ldc );
      break;
    }
    case BLIS_DCOMPLEX :
    {
      ...
      cblas_zhemm( cblas_order, cblas_side, cblas_uplo, mm, nn, alphap,
                                      ap, lda, bp, ldb, betap, cp, ldc );
      break;
    }
    default :
...
double blas_hemm(
  ...
  switch( dt )  {
    ...
    case BLIS_SCOMPLEX :
    {
      ...
      scomplex*  betap  = (scomplex*) bli_obj_buffer( beta );
      scomplex*  cp     = (scomplex*) bli_obj_buffer( c );
      chemm_( &f77_side, &f77_uploa, &mm, &nn, alphap, ap, (f77_int*)&lda,
                           bp, (f77_int*)&ldb, betap, cp, (f77_int*)&ldc );
      break;
    }
    case BLIS_DCOMPLEX :
    {
      ...
      dcomplex*  betap  = (dcomplex*) bli_obj_buffer( beta );
      dcomplex*  cp     = (dcomplex*) bli_obj_buffer( c );
      zhemm_( &f77_side, &f77_uploa, &mm, &nn, alphap, ap, (f77_int*)&lda,
                           bp, (f77_int*)&ldb, betap, cp, (f77_int*)&ldc );
      break;
    }
    default :
  ...
  if(params->api == API_BLIS) {
   ...
   if( bli_obj_row_stride( c ) == 1 ) {
    ...
    if(params->api == API_CBLAS) {
      cblas_hemm( side, uploa, mm, nn, alpha, a, lda, b, ldb, beta, c, ldc, dt );
    } else { /**/
      if( bli_obj_row_stride( c ) == 1 ) {
        blas_hemm( side, uploa, mm, nn, alpha, a, lda, b, ldb, beta, c, ldc, dt );
      }
      else {
        if( side == BLIS_LEFT)
          side = BLIS_RIGHT;
        ...
        blas_hemm( side, uploa, nn, mm, alpha, a, lda, b, ldb, beta, c, ldc, dt );
      }
    }
  }
  return ;
...
double libblis_ref_hemm(
  ...
  if ((params->nanf) && (*betap == 0)) {
    resid = libblis_check_nan_hemm(c, dt );
  }
  else if((params->bitextf == 0) && (params->oruflw == BLIS_DEFAULT)) {
	   resid = libblis_test_hemm_check( params, side, alpha, a, b, beta, c, c_save );
  }
  else {
    if(params->oruflw == BLIS_DEFAULT) {
	     resid = libblis_test_ihemm_check( params, side, alpha, a, b, beta, c, c_save );
    }
    else {
...
double libblis_test_bitrp_hemm(
  test_params_t* params,
  iface_t        iface,
  side_t         side,
  obj_t*         alpha,
  ...
  for(i = 0; i < n_repeats; i++) {
    bli_copym( c_orig, r );
		  libblis_test_hemm_impl( iface, side, alpha, a, b, beta, r );
...
double libblis_test_op_hemm (
  ...
  libblis_api_hemm(params, iface, side, &alpha, &a, &b, &beta, &c, datatype );
  ...
  if(params->bitrp) {
    ...
    libblis_test_mobj_create( params, datatype, BLIS_NO_TRANSPOSE,
                                                sc_str[0], m, n, &r );
    resid = libblis_test_bitrp_hemm( params, iface, side,&alpha, &a, &b,
                                            &beta, &c, &c_save, &r, datatype);
    bli_obj_free( &r );
  }
  else {
    resid = libblis_ref_hemm(params, side, &alpha, &a, &b, &beta,
                                              &c, &c_save, datatype );
  }
#endif
