...
double libblis_test_hemv_check(
  test_params_t* params,
  obj_t*         alpha,
  obj_t*         a,
  obj_t*         x,
  ...
  if(params->api == API_BLIS) {
   ...
   if ( bli_obj_row_stride( a ) == 1 ) {
    ...
    if(params->api == API_CBLAS) {
     	cblas_hemv(uploa, mm, alpha, a, lda, x, incx, beta, y, incy, dt );
    }
    else { /**/
      f77_char f77_uploa;
      if ( bli_obj_row_stride( a ) == 1 ){
        bli_param_map_blis_to_netlib_uplo( uploa, &f77_uploa );
        blas_hemv(f77_uploa, mm, alpha, a, lda, x, incx, beta, y, incy, dt );
      }
      else {
        if( uploa == BLIS_UPPER)
          uploa = BLIS_LOWER;
...
double libblis_ref_hemv(
  ...
  if((params->bitextf == 0) && (params->oruflw == BLIS_DEFAULT)) {
    resid = libblis_test_hemv_check( params, alpha, a, x, beta, y, y_orig );
  }
  else {
    if(params->oruflw == BLIS_DEFAULT) {
      resid = libblis_test_ihemv_check( params, alpha, a, x, beta, y, y_orig );
    }
    else {
...
double libblis_test_bitrp_hemv(
  test_params_t* params,
  iface_t        iface,
  obj_t*         alpha,
  obj_t*         a,
  ...
  for(i = 0; i < n_repeats; i++) {
    bli_copyv( y_orig, r );
    libblis_test_hemv_impl( iface, alpha, a, x, beta, r );
...
double libblis_test_op_hemv (
  ...
  bli_obj_set_struc( BLIS_HERMITIAN, &aa );
  ...
  libblis_api_hemv(params, iface, &alpha, &aa, &xx, &beta, &y );
  ...
  if(params->bitrp) {
    ...
    resid = libblis_test_bitrp_hemv( params, iface, &alpha, &a, &x,
                                   &beta, &y, &y_save, &r, datatype);

    bli_obj_free( &r );
  }
  else {
    resid = libblis_ref_hemv( params, &alpha, &a, &x, &beta, &y, &y_save );
  }
#endif
