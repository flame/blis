  ...
  if(params->api == API_BLIS) {
   ...
   if ( bli_obj_row_stride( a ) == 1 ) {
    ...
    if(params->api == API_CBLAS) {
     	cblas_her2(uploa, mm, alpha, x, incx, y, incy, a, lda, dt );
    }
    else { /**/
      f77_char f77_uploa;
      if ( bli_obj_row_stride( a ) == 1 ){
        bli_param_map_blis_to_netlib_uplo( uploa, &f77_uploa );
        blas_her2(f77_uploa, mm, alpha, x, incx, y, incy, a, lda, dt );
      }
      else {
        if( uploa == BLIS_UPPER)
          uploa = BLIS_LOWER;
        ...
        bli_param_map_blis_to_netlib_uplo( uploa, &f77_uploa );
        blas_her2(f77_uploa, mm, alpha, y, incy, x, incx, a, lda, dt );
      }
    }
  }
  return ;
...
double libblis_ref_her2(
  ...
  if((params->bitextf == 0) && (params->oruflw == BLIS_DEFAULT)) {
    resid = libblis_test_her2_check( params, alpha, x, y, a, a_orig);
  }
  else {
    if(params->oruflw == BLIS_DEFAULT) {
      resid = libblis_test_iher2_check( params, alpha, x, y, a, a_orig);
    }
    else {
...
double libblis_test_bitrp_her2(
  test_params_t* params,
  iface_t        iface,
  obj_t*         alpha,
  obj_t*         x,
  ...
  for(i = 0; i < n_repeats; i++) {
    ...
    bli_mkherm( r );
    bli_mktrim( r );
    libblis_test_her2_impl( iface, alpha, x, y, r );
...
double libblis_test_op_her2 (
  ...
  libblis_api_her2(params, iface, &alpha, &xx, &yy, &a, datatype );
  ...
  if(params->bitrp) {
    ...
    resid = libblis_test_bitrp_her2( params, iface, &alpha, &x, &y,
                                                &a, &a_save, &r, datatype);

    bli_obj_free( &r );
  }
  else {
    resid = libblis_ref_her2( params, &alpha, &x, &y, &a, &a_save );
  }
#endif
