...
void cblas_trmv(
  ...
  if(bli_is_unit_diag(diaga))
    ...
    case BLIS_FLOAT :
    {
      ...
      float*   xp     = (float*) bli_obj_buffer( x );
      cblas_strmv(cblas_order, cblas_uploa, cblas_transa, cblas_diaga,
                                                m, ap, lda, xp, incx );
      break;
    }
    case BLIS_DOUBLE :
    {
      ...
      double*  xp     = (double*) bli_obj_buffer( x );
      cblas_dtrmv(cblas_order, cblas_uploa, cblas_transa, cblas_diaga,
                                                m, ap, lda, xp, incx );
      break;
    }
    case BLIS_SCOMPLEX :
    {
      ...
      scomplex*  xp     = (scomplex*) bli_obj_buffer( x );
      cblas_ctrmv(cblas_order, cblas_uploa, cblas_transa, cblas_diaga,
                                                m, ap, lda, xp, incx );
      break;
    }
    case BLIS_DCOMPLEX :
    {
      ...
      dcomplex*  xp     = (dcomplex*) bli_obj_buffer( x );
      cblas_ztrmv(cblas_order, cblas_uploa, cblas_transa, cblas_diaga,
                                                m, ap, lda, xp, incx );
      break;
    }
    default:
...
void blas_trmv(
  ...
  switch( dt )  {
    case BLIS_FLOAT :
    {
      float*   ap     = (float*) bli_obj_buffer( a );
      float*   xp     = (float*) bli_obj_buffer( x );
      strmv_(&f77_uploa, &f77_transa, &f77_diaga, &m, ap, (f77_int*)&lda, xp, &incx);
      break;
    }
    case BLIS_DOUBLE :
    {
      double*  ap     = (double*) bli_obj_buffer( a );
      double*  xp     = (double*) bli_obj_buffer( x );
      dtrmv_(&f77_uploa, &f77_transa, &f77_diaga, &m, ap, (f77_int*)&lda, xp, &incx);
      break;
    }
    case BLIS_SCOMPLEX :
    {
      scomplex*  ap     = (scomplex*) bli_obj_buffer( a );
      scomplex*  xp     = (scomplex*) bli_obj_buffer( x );
      ctrmv_(&f77_uploa, &f77_transa, &f77_diaga, &m, ap, (f77_int*)&lda, xp, &incx);
      break;
    }
    case BLIS_DCOMPLEX :
    {
      dcomplex*  ap     = (dcomplex*) bli_obj_buffer( a );
      dcomplex*  xp     = (dcomplex*) bli_obj_buffer( x );
      ztrmv_(&f77_uploa, &f77_transa, &f77_diaga, &m, ap, (f77_int*)&lda, xp, &incx);
      break;
    }
    default:
      bli_check_error_code( BLIS_INVALID_DATATYPE );
...
void libblis_api_trmv(
  ...
  if(params->api == API_BLIS) {
   ...
   if ( bli_obj_row_stride( a ) == 1 ) {
    ...
    libblis_trmv_alphax(alpha, mm, x, incx, dt);
    ...
    if(params->api == API_CBLAS) {
     	cblas_trmv(uploa, transa, diaga, mm, a, lda, x, incx, dt );
    }
    else { /**/
      if ( bli_obj_row_stride( a ) == 1 ){
        blas_trmv(uploa, transa, diaga, mm, a, lda, x, incx, dt );
      }
      else {
        if( uploa == BLIS_UPPER)
          uploa = BLIS_LOWER;
...
double libblis_ref_trmv(
  ...
  if((params->bitextf == 0) && (params->oruflw == BLIS_DEFAULT)) {
    resid = libblis_test_trmv_check( params, alpha, a, x, x_orig );
  }
  else {
    if(params->oruflw == BLIS_DEFAULT) {
      resid = libblis_test_itrmv_check( params, alpha, a, x, x_orig );
    }
    else {
...
double libblis_test_bitrp_trmv(
  test_params_t* params,
  iface_t        iface,
  obj_t*         alpha,
  obj_t*         a,
  ...
  for(i = 0; i < n_repeats; i++) {
    bli_copyv( x_orig, r );
    libblis_test_trmv_impl( iface, alpha, a, r);
...
double libblis_test_op_trmv (
  ...
  libblis_api_trmv(params, iface, &alpha, &aa, &x, datatype);
  ...
  if(params->bitrp) {
    ...
    resid = libblis_test_bitrp_trmv( params, iface, &alpha, &a, &x,
                                               &x_save, &r, datatype);

    bli_obj_free( &r );
  }
  else {
    resid = libblis_ref_trmv( params, &alpha, &a, &x, &x_save );
  }
#endif
