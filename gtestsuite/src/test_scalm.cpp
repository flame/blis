...
double libblis_ref_scalm(
  ...
  if((params->bitextf == 0) && (params->oruflw == BLIS_DEFAULT)) {
    resid = libblis_test_scalm_check( params, beta, y, y_orig );
  }
  else {
    if(params->oruflw == BLIS_DEFAULT) {
      resid = libblis_test_iscalm_check( params, beta, y, y_orig);
    }
    else {
...
double libblis_test_bitrp_scalm(
  test_params_t* params,
  iface_t        iface,
  obj_t*         beta,
  obj_t*         y,
  ...
  for(i = 0; i < n_repeats; i++) {
    bli_copym( y_orig, r );
    libblis_test_scalm_impl( iface, beta, r );
...
double libblis_test_op_scalm (
  ...
  if(params->bitrp) {
    ...
    resid = libblis_test_bitrp_scalm( params, iface, &beta, &y,
                                                &y_save, &r, datatype);

    bli_obj_free( &r );
  }
  else {
    resid = libblis_ref_scalm( params, &beta, &y, &y_save );
  }
#endif
