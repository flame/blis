...
// Zero out the vector registers that will be used for the A*B product.
...
// Preload 1 column of A and 1 row of B.
#define PRELOAD_A_B \
"lxv           %%vs36, 0(%%r7)                    \n\t" \
"lxv           %%vs37, 16(%%r7)                   \n\t" \
"lxv           %%vs38, 32(%%r7)                   \n\t" \
...
/* 
  Compute the A*B product using column then row herring bone technique.
  Inner loop is unrolled by 4.
...
/* 
  Compute the A*B product going in order of columns of the microtile.
  Inner loop is unrolled by 16.
...
// Scale the A*B product by alpha.
#define SCALE_ALPHA \
...
// Scale C matrix by beta. Scales 3 columns of C at a time. Used for when C is
// stored by columns.
...
// Load C matrix. Used for when C is stored by columns.
#define DCOL_LOAD_C \
"lxv              %%vs36, 0(%%r22)                \n\t" \
"lxv              %%vs37, 16(%%r22)               \n\t" \
"lxv              %%vs38, 32(%%r22)               \n\t" \
...
// Scale C matrix by beta. Scales 1 column at a time. Used for when C is
// stored generally.
...
// Load C matrix. Used for when C is stored generally.
#define DGEN_LOAD_C \
"lxsdx	     %%vs36, %%r9, %%r22                  \n\t" \
"lxsdx       %%vs37, 0, %%r22                     \n\t" \
...
// Increment row offsets by a column stride.
...
// Load, scale, and increment offsets of the C matrix. Used for when C is
// stored generally.
...
// Store C = beta*C + alpha*A*B into memory.
#define DCOL_STORE \
"stxv              %%vs0, 0(%%r16)                \n\t" \
"stxv              %%vs1, 16(%%r16)               \n\t" \
"stxv              %%vs2, 32(%%r16)               \n\t" \
